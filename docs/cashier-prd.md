...
Status: All 7 phases built — pending SQL scripts to run live
...
- ✅ **Phase 7** (done ahead of order — shared build with Branch Manager and Branch Field Collector PRDs) — New `/leave-requests` page: self-service leave request + balance (reuses the schema's existing but previously-unused `employees.paid_leaves_used` and `settings.paid_leaves_annual`), with Branch Manager/Admin approval. `/employee-loans` now scoped: non-approvers only see and apply for their own loans, approvers keep the full list. **DB migration pending** — run `supabase/add_leave_requests.sql` (confirmed 404 live).
- ✅ **Phase 6** — Collateral Monitoring. New "Collateral" card on the loan detail page (`/loans/[id]`): Cashier/Admin can add an item (ORCR / Bank Check / Other, reference #, description, notes) and mark it released (captures who it was released to + date). Everyone else can view. Lifecycle kept simple — held → released only, as scoped in the PRD; let me know if you need more states (partial release, damaged, disputed). **DB migration pending** — run `supabase/add_collateral.sql` (confirmed 404 live).
- ✅ **Phase 5** — General Ledger. New `/general-ledger` page (Cashier/Accounting/Admin) with 4 tabs: Journal Entries (manual multi-line entry, debit=credit enforced before save), Chart of Accounts (11 default accounts seeded — Admin can add more), Income Statement (date range), Balance Sheet (as-of date, with a computed "Retained Earnings" plug from revenue−expense so Assets = Liabilities + Equity). Auto-posts on: a payment (Debit Cash, Credit Loans Receivable — simplified, doesn't split principal/interest since those columns aren't tracked at post time), a loan disbursement (Debit Loans Receivable for the full amount, Credit Cash for the release amount, Credit Service Fee Income for the fee — these balance exactly), and a recorded expense (Debit Operating Expenses, Credit Cash). Auto-posting never blocks the primary action if it fails. **Not auto-posted:** manual Cash Flow entries on `/accounting` — their category is free text, so there's no reliable account to post to automatically; log them as a manual journal entry if needed. **DB migration pending** — run `supabase/add_general_ledger.sql`.
- ✅ **Item #7 reopened** — Cashier can now create customers (was Admin-only from Phase 1). Document upload moved from the loan-approval dialog to customer creation: after saving a new customer, the same dialog switches to a document-upload step (Valid ID, Barangay Clearance, Proof of Billing, Promissory Note). The Approve-loan dialog now shows the same checklist read-only (view/verify only) with a link to the customer's profile if anything's missing, instead of letting you upload from there. **DB migration pending** — run `supabase/allow_cashier_create_customers.sql`.
- ⬜ All phases done.
